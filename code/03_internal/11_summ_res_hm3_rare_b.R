...
##############################
## summ function
summPheno <- function(test_pheno, chr_path, test_cov, lm_coef){
  
  require(bigreadr)
  require(pROC)
  pred_tot <- vector()
  for(chr in 1: 22){
    pred_chr_str <- paste0(chr_path, chr, ".profile.gz")
...
summPheno_val <- function(valid_pheno, chr_path, val_cov){
  ...
  pred_tot <- vector()
  for(chr in 1: 22){
    pred_chr_str <- as.character(paste0(chr_path, chr, ".profile.gz"))
...
method <- c("CT", "DBSLMM/rare", "lassosum",
            "LDpred2", "LDpred2", "LDpred2", "LDpred2", 
             "SCT")
situ <- c("_hm3_rare", "_hm3_rare_best", "_hm3_rare", 
          "_auto_hm3_rare", "_inf_hm3_rare", "_nosp_hm3_rare", "_sp_hm3_rare", 
          "_hm3_rare")
...
for (m in c(2)){
  ...
  for (cross in 1: 5){
    ...
    lm_coef <- try(summPheno_val(val_pheno, val_chr_path, val_cov_eff), silent = T)
    temp_res <- try(summPheno(test_pheno_na, chr_path, test_cov_eff[, 1], lm_coef), silent = T)
...
load(paste0(comp_str, "06_internal_b/summRes/res_hm3_rare_b_pheno", p, ".RData"))
save(res_list, file = paste0(comp_str, "06_internal_b/summRes/res_hm3_rare_b_pheno", p, "_old.RData"))
res_list[[1]][,2] <- pr2_mat[,2]
res_list[[2]][,2] <- auc_mat[,2]

# res_list <- list(pr2 = pr2_mat, 
#                  auc = auc_mat)
save(res_list, file = paste0(comp_str, "06_internal_b/summRes/res_hm3_rare_b_pheno", p, ".RData"))
