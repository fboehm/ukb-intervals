...
ancestry_info_AFR <- ancestry_info[ancestry_info[, 2] %in% c(4, 2001, 2002, 4001, 4002, 4003), ]
ancestry_info_AFR <- ancestry_info_AFR[match(eid_i, ancestry_info_AFR$eid), ]
ancestry_info_AFR <- ancestry_info_AFR[cnd, ]
ancestry_info_AFR <- ancestry_info_AFR[!is.na(ancestry_info_AFR[, 1]), ]
cat("Sample size of African ancestry: ", nrow(ancestry_info_AFR), ".\n")
...
pheno_c_code1 <- c("eid", 
                   ...
                   # paste0(23115, "-0.0"),   # left fat percentage (LFP)
...
## match data
pheno_c_AFR <- pheno_c[match(ancestry_info_AFR$eid, pheno_c$eid), ]
sqc_i_AFR <- sqc_i[match(ancestry_info_AFR$eid, sqc_i$eid), ]
save(sqc_i_AFR, file = "/net/mulan/disk2/yasheng/comparisonProject/02_pheno/08_pheno_AFR/01_sqc.RData")
save(pheno_c_AFR, file = "/net/mulan/disk2/yasheng/comparisonProject/02_pheno/08_pheno_AFR/02_pheno_c_raw.RData")
...
pheno_b <- fread2("/net/mulan/data/UKB/ukb10683.csv", select = c("eid", 
                                                                 ...
                                                                 pheno_b_code))
## match data
pheno_b_AFR <- pheno_b[match(ancestry_info_AFR$eid, pheno_c$eid), ]
save(pheno_b_AFR, file =  "/net/mulan/disk2/yasheng/comparisonProject/02_pheno/08_pheno_AFR/03_pheno_b_raw.RData")

## idx
idx1 <- cbind(sqc_i_AFR$idx, sqc_i_AFR$idx)
idx2 <- rbind(c(1, 1), idx1)
write.table(idx2, 
            file = "/net/mulan/disk2/yasheng/comparisonProject/02_pheno/08_pheno_AFR/idx_AFR1.txt", 
            row.names = F, col.names = F, quote = F)
write.table(idx1, 
            file = "/net/mulan/disk2/yasheng/comparisonProject/02_pheno/08_pheno_AFR/idx_AFR2.txt", 
            row.names = F, col.names = F, quote = F)
