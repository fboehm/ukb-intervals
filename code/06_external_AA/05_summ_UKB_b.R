...
summPheno <- function(chr_path, test_cov, lm_coef, type){
  ...
  require(bigreadr)
  require(pROC)
  test_pheno <- pheno_b_AFR_all[-3594, p]
  test_cov <- test_cov[-3594]
  pred_tot <- vector()
  for(chr in 1: 22){
  ...
  pheno_tot <- pheno_tot[!is.na(test_pheno)]
  test_cov <- test_cov[!is.na(test_pheno)]
  test_pheno <- test_pheno[!is.na(test_pheno)]
  pred <- cbind(cbind(1, pheno_tot), test_cov) %*% lm_coef
  auc <- as.numeric(roc(test_pheno, pred[, 1], levels = c(0, 1))$auc)
  pr2 <- as.numeric(lrm(test_pheno ~ pred[, 1])$stats["R2"])
  result <- list(auc, pr2)
  return(result)
}
...
test_cov_eff <- fread2(paste0(comp_str, "02_pheno/08_pheno_AFR/coveff_pheno", p, ".txt"))[, 1]
...
pheno_path1 <- paste0(comp_str, "07_external_c/01_AFR/03_res/binary/")
pheno_path2 <- paste0(comp_str, "06_internal_b/")
auc_mat <- pr2_mat <- matrix(NA, ncol = length(method), nrow = 5)
...
save(auc_mat, file = paste0(comp_str, "07_external_c/01_AFR/04_summ_res/res_hm3_b_pheno",
                            p, "_dat1.RData"))
