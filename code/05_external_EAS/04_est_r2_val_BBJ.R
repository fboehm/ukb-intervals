...
est_chr_r2 <- function(summ_dat, est_dat, chr){
  ...
  load(paste0(comp_str, "09_external_LD/EAS/LD_mat", opt$cross, "/chr", chr, ".RData"))
...
summ <- fread2(paste0(comp_str, "07_external_c/03_EAS/02_clean/pheno", opt$pheno,
...
for (chr in 1: 22){
  ...
  if (opt$method %in% c("CT", "SCT", "lassosum")){
    ...
    out_str <- paste0(comp_str, "07_external_c/03_EAS/03_res/continuous/pheno", opt$pheno,
  ...
  if (opt$method == "DBSLMM"){
    est_str <- paste0(comp_str, "07_external_c/03_EAS/03_res/continuous/pheno", opt$pheno,
                      "_data1/DBSLMM/summary_hm3_cross", opt$cross, "_chr", chr, "_best.dbslmm.txt")
    est <- fread2(est_str)
    est2 <- data.frame(SNP = est[, 1], EA = est[, 2], 
                       beta = est[, 3])
    out_str <- paste0(comp_str, "07_external_c/03_EAS/03_res/continuous/pheno", opt$pheno,
  ...
  if (opt$method == "sblup"){
    ...
    out_str <- paste0(comp_str, "07_external_c/03_EAS/03_res/continuous/pheno", opt$pheno,
                      "_data", opt$data, "/", opt$method, "/val_r2_hm3_cross", opt$cross, ".txt")
  }
  ...
  if (opt$method %in%  "auto"){
    ...
    out_str <- paste0(comp_str, "07_external_c/pheno", opt$pheno,
