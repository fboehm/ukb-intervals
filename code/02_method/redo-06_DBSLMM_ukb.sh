...
#SBATCH --job-name=redo-DBSLMM
#SBATCH --mem=12G
#SBATCH --cpus-per-task=5
#SBATCH --array=1
#SBATCH --output=/net/mulan/home/fredboe/research/ukb-intervals/cluster_outputs/redo-06_DBSLMM_ukb_%a.out
#SBATCH --error=/net/mulan/home/fredboe/research/ukb-intervals/cluster_outputs/redo-06_DBSLMM_ukb_%a.err


let k=0
let thread=${SLURM_CPUS_PER_TASK}
# we see that ~/research/ukb-intervals/study_nfolds/10-fold/05_internal_c/pheno14/DBSLMM/pred_ukb_best_cross1_chr1.profile' does not exist.
        ...
        for p in 14; do
            for cross in `seq 1 ${nfold}`; do
                ...
                if [ ${k} -eq ${SLURM_ARRAY_TASK_ID} ]; then
                    ...
                    mkdir -p ${outPath}
                    outfile=${outPath}summary_ukb_cross${cross}_chr${chr}_best.dbslmm.txt
                    ## DBSLMM
                    if [ ! -f ${outfile} ]; then 
