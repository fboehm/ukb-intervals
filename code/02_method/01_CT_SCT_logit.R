...
# opt <- list(summ = "/net/mulan/disk2/yasheng/comparisonProject/06_internal_b/pheno1/output/summary_hm3_cross1.assoc.logistic",
#             path = "/net/mulan/disk2/yasheng/comparisonProject/06_internal_b/pheno1/",
#             thread = 4,
#             pheno = 1,
#             cross = 1,
#             dat = "binary",
...
summstats <- fread2(opt$summ, select =  c(1, 2, 3, 4, 7, 8, 9))
colnames(summstats) <- c("chr", "rsid", "pos", "a1", "OR", "t", "pval") # calculate P
summstats$beta <- log(summstats$OR)
p_val <- ifelse(summstats$t < 0, 
                pnorm(summstats$t), 
                pnorm(summstats$t, lower.tail = F))*2
summstats$pval <- ifelse(p_val == 0,
                         min(p_val[-which(p_val==0)]),
                         p_val)

merge_bim <- fread2("/net/mulan/home/yasheng/predictionProject/plink_file/hm3/merge.bim")
summstats$a0 <- merge_bim[match(summstats[,2], merge_bim[,2]),6]
...
ct_bk_str <- ifelse(opt$dat == "continuous",
                    paste0(comp_str, "05_internal_c/pheno", opt$pheno, 
...
# output
write.table(grid2, file = paste0(opt$path, "CT/grid2_", opt$reftype, "_cross", opt$cross, ".logistic"),
            col.names = F, row.names = F, quote = F)
write.table(snp_sig_CT, file = paste0(opt$path, "CT/esteff_", opt$reftype, "_cross", opt$cross, ".logistic"),
            col.names = F, row.names = F, quote = F)
write.table(snp_sig_SCT, file = paste0(opt$path, "SCT/esteff_", opt$reftype, "_cross", opt$cross, ".logistic"),
            col.names = F, row.names = F, quote = F)
