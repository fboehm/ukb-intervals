...
while getopts "D:p:B:s:m:T:H:G:R:o:P:l:c:i:t:C:f:h:" opt; do
  case $opt in
...
printf "\033[33mArgument h2f is %s  \033[0m\n" "$h2f"
printf "\033[33mArgument pth is %s  \033[0m\n" "$pth"

DBSLMM=${software_path}DBSLMM/software/DBSLMM_val.R
	...
	# do
		...
		Rscript ${DBSLMM} --summary ${summchr}.assoc.txt --outPath ${outpath} --plink ${plink} --model ${model}\
						  --dbslmm ${dbslmm} --ref ${ref_geno} --n ${n} --type ${type} --nsnp ${nsnp} --block ${BLOCK}.bed\
						   --h2 ${h2} --h2f ${h2f} --pth ${pth} --thread ${thread} --val ${val_geno}
