...
if (trait_type == "continuous"){
  fam <- readr::read_table(file = "~/research/ukb-intervals/plink_file/ukb/continuous/chr1.fam", col_names = FALSE)
} else {
  fam <- readr::read_table(file = "~/research/ukb-intervals/plink_file/ukb/binary/chr1.fam", col_names = FALSE)
}
# define cols to keep
...
fam_all <- fam %>%
              ...
              dplyr::mutate(true_pheno = 
                purrr::pmap_dbl(.l = list(fold1, fold2, fold3, fold4, fold5), 
...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:22){
    if (trait_type == "continuous"){
      ...
      fn <- paste0("~/research/ukb-intervals/06_internal_b/pheno", 
                  phe_num, 
                  "/DBSLMM/pred_ukb_best_cross", 
                  cross_num, 
                  "_chr", 
  ...
  # read idx file
  if (trait_type == "continuous") {
    idx_fn <- paste0("~/research/ukb-intervals/02_pheno/01_test_idx_c/idx_pheno", phe_num, "_cross", cross_num, ".txt")
  } else {
    idx_fn <- paste0("~/research/ukb-intervals/02_pheno/04_test_idx_b/idx_pheno", phe_num, "_cross", cross_num, ".txt")
  }
...
if (trait_type == "continuous"){
  fn <- paste0("~/research/ukb-intervals/03_subsample/continuous/pheno", phe_num, "/verif/03_idx_pheno.txt")
} else {
  fn <- paste0("~/research/ukb-intervals/03_subsample/binary/pheno", phe_num, "/verif/03_idx_pheno.txt")
}
  ...
  for (chr_num in 1:22){
    if (chr_num == 1){
      s2[[chr_num]] <- out[[cross_num]][[chr_num]] %>%
        dplyr::select(1, 6)  # choose only the SCORESUM column
        colnames(s2[[chr_num]])[2] <- paste0(colnames(s2[[chr_num]])[2], "_chr", chr_num) 
    } else {
      s2[[chr_num]] <- out[[cross_num]][[chr_num]] %>%
...
for (i in 1:n_verif){
  ...
  o3 <- out_tib %>% 
            dplyr::mutate(fitted_cv =  purrr::map_dbl(.x = cross_membership, 
                                                      .f = get_fitted, 
                                                      fitted_vec = as.vector(unlist(pred_verification_set_tib[i, 2:6])))
...
mytib <- intervals %>% 
  dplyr::bind_rows() #%>%
saveRDS(object = mytib, file = paste0(trait_type, "_ints.rds"))
# need a join with the idx_pheno object containing true phenotype values
mytib2 <- mytib %>%
  dplyr::left_join(verif_true_pheno, by = c("FID" = "X1")) %>%
