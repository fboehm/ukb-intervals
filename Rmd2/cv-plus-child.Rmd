...
if (trait_type == "continuous"){
  ...
  fn <- paste0("~/research/ukb-intervals/03_subsample/binary/pheno", phe_num, "/verif/01_idx.txt")
}
verif_true_pheno <- readr::read_table(fn, col_names = FALSE) %>%
                              dplyr::arrange(X1)
# load true phenotypes for all subjects
...
```{r}
# read idx_pheno file for trait of interest - for verification set
vfn <- paste0(comp_str, 
              "03_subsample/continuous/pheno", 
              phe_num,
              "/verif/03_idx_pheno.txt")
v_tib <- vroom::vroom(vfn, col_names = FALSE)
```
