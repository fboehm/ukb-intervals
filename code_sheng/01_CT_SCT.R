...
ct_bk_str <- ifelse(opt$dat == "continuous",
                    paste0(fb_str, "05_internal_c/pheno", opt$pheno, 
                           "/CT/summary_cross", opt$cross, "_sub-",
                           as.numeric(as.POSIXlt(Sys.time()))),
                    paste0(fb_str, "06_internal_b/pheno", opt$pheno, 
                           "/CT/summary_cross", opt$cross, "_sub-",
                           as.numeric(as.POSIXlt(Sys.time()))))
...
if (opt$dat == "continuous"){
  y <- fread2(paste0(fb_str, "03_subsample/continuous/pheno", opt$pheno, 
                     "/val/",opt$reftype,"/02_pheno_c.txt"))[, 1]
} else {
  y <- fread2(paste0(fb_str, "03_subsample/binary/pheno", opt$pheno, 
                     "/val/02_pheno_b.txt"))[, 1]
  covar <- fread2(paste0(fb_str, "03_subsample/binary/pheno", opt$pheno, 
...
# # output
write.table(grid2, file = paste0(opt$path, "CT/grid2_", opt$reftype, "_cross", opt$cross, ".txt"),
             col.names = F, row.names = F, quote = F)
 write.table(snp_sig_CT, file = paste0(opt$path, "CT/esteff_", opt$reftype, "_cross", opt$cross, ".txt"),
             col.names = F, row.names = F, quote = F)
 write.table(snp_sig_SCT, file = paste0(opt$path, "SCT/esteff_", opt$reftype, "_cross", opt$cross, ".txt"),
             col.names = F, row.names = F, quote = F)
