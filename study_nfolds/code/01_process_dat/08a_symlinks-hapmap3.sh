...
for nfold in ${nfolds[@]}; do
    ...
    for chr in `seq 1 22`; do
        ...
        ln -s ${orig_path}chr${chr}.bed ${fred_path}binary/chr${chr}.bed
        ln -s ${orig_path}chr${chr}.bim ${fred_path}binary/chr${chr}.bim
        ln -s ${fam_path}binary/chr1.fam ${fred_path}binary/chr${chr}.fam
