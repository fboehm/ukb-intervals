...
args_list = list(
  ...
  make_option("--nfold", type="numeric", default=5, help="INPUT: number of folds for the analysis", metavar = "character")
)
...
if (opt$dat == "binary"){
  ...
  val_str <- paste0(comp_str, "study_nfolds/", opt$nfold, 
                    "-fold/03_subsample/binary/pheno", opt$pheno, 
                    "/val/hm3/geno/")
...
if (opt$dat == "continuous"){
  ref_str <- paste0(comp_str, "04_reference/hm3/continuous/geno/")
  val_str <- paste0(comp_str, "study_nfolds/", opt$nfold, 
                    "-fold/03_subsample/continuous/pheno", opt$pheno, 
                    "/val/hm3/geno/")
