...
phe_num <- x
fam_file <- here::here("dat", "simulations-ding", paste0("gemma_hsq", hsq, "_pcausal", pcausal), "chr1.fam")
  # read fam file
...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:22){
    ...
    profile_fn <- here::here("dat", "simulations-ding", "DBSLMM_hsq0.2_pcausal0.001", pfn)
...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:22){
    ...
    fn <- here::here("dat", "simulations-ding", "verification", paste0("allele-scores_hsq", hsq, "_pcausal", pcausal), paste0("pred_ukb_pheno", phe_num, "_fold", cross_num, "_chr", chr_num, "_best.dbslmm.txt.profile"))
...
pred_vals_verification_set_tib <- do.call("cbind", pred_vals_verification_set) %>%
  tibble::as_tibble() #%>%
  dplyr::mutate(true_pheno = verification_true_pheno_tib$X1)
```
