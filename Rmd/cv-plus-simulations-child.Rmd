...
phe_num <- x
fam_file <- here::here("dat/simulations-ding/gemma/chr1.fam")
for (cross_num in 1:5){
  ...
  for (chr_num in 1:22){
    ...
    profile_fn <- here::here("dat", "simulations-ding", "DBSLMM", pfn)
...
subjects belonging to the validation set.


```{r, results='hide', message = FALSE}
val_fn <- here::here("dat", "simulations-ding", "validation", paste0("pheno", phe_num, ".txt"))
validation_true_pheno_tib <- readr::read_table(val_fn, col_names = FALSE)
```
...
for (cross_num in 1:5){
  ...
  for (chr_num in 1:22){
    ...
    fn <- paste0("~/research/ukb-intervals/dat/simulations-ding/validation/allele-scores/pred_ukb_pheno", phe_num, "_fold", cross_num, "_chr", chr_num, "_best.dbslmm.txt.profile")
...
n_validation <- nrow(pred_vals_validation_set_tib)
intervals <- list()
